...
사다리타기 - 한 번 지나간 다리부분을 부숴버리면(`arr[x][y] = -1`) 무한루프 x  
...
def convex_down(s, e, start):
    ...
    for i in range(start, N):
        ...
        while True:
            print(stack)
            nowX, nowY, nowSlope = stack[-1]
            nextSlope = (nextY - nowY) / (nextX - nowX) if nextX - nowX else (INF if nextY-nowY > 0 else -INF)

            if nextSlope > nowSlope:
                stack.append((nextX, nextY, nextSlope))
                break

            elif nextSlope <= nowSlope:
                stack.pop()

    return stack


def convex_up(s, e, start):
    stack = [(s, e, -INF)]
...
print(arr)
print()
print(convex_up(stack[-1][0], stack[-1][1], 2))
